...
The code used to produce this document can be accessed on [github](https://github.com/willdrysdale/reveal), and used merged FAAM data which can be found in the repo [here]() - note no correction has been applied to this merge to account for the time lag between the different instrument inlets. The versions of the data on CEDA should always be considered the authoritative copies. The github repo includes code for downloading the data directly from CEDA using the [faamr](https://github.com/wacl-york/faamr) package.
